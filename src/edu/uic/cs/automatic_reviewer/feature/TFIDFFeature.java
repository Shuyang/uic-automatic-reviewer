...
public class TFIDFFeature extends AbstractWordOperations{
	...
	public void extractIDF(List<Paper> papers){
		...
		for(Paper p:papers){
			...
			List<String> termList = porterStemmingAnalyzeUsingDefaultStopWords(p.getTitle());
		...
		for(Map.Entry<String, Double> entry:df.entrySet()){
			...
			if(term_df >= Constants.MIN_FREQUENCY_OF_TERMS){
...
//			System.out.println(e.getKey() + " "+ e.getValue() + " " + id_idf.get(e.getValue()));
//		}
	}
	
	
	public ArrayList<ArrayList<svm_node>> tfidfForAllTerms(List<Paper> papers, int offset){
		int n = papers.size();
		int m = id_idf.size();
