...
public class MetaDataFeatures {
	...
	public static ArrayList<ArrayList<svm_node>> extractFeatureFromMetaDataMap(
			List<Paper> papers, FeatureExtractor.FeatureType type, int offset) {
		int n = papers.size();
		int m = Constants.Feature.MAX_NUMBER_OF_PAGES_PER_PAPER;
		ArrayList<ArrayList<svm_node>> features = new ArrayList<ArrayList<svm_node>>();

		for (int i = 0; i < n; ++i) {
			Paper p = papers.get(i);
			Map<Integer, Integer> pageMap = MapFromMetaData(p.getMetadata(),
					type);
	...
	private static Map<Integer, Integer> MapFromMetaData(Metadata m,
		...
		case NumOfFiguresByPage:
			return m.getNumOfFiguresByPage();
		case NumOfTablesByPage:
			return m.getNumOfTablesByPage();
		case NumOfFormulasByPage:
			return m.getNumOfFormulasByPage();
		default:
			return null;
		}
	}
