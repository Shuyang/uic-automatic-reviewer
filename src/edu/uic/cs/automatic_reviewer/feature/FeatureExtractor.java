...
public class FeatureExtractor {
	...
	private int feautreSize(FeatureType type){
		...
		case NumOfFormulasByPage: 
			return Constants.MAX_NUMBER_OF_PAGES_PER_PAPER;
		case TFIDF: 
			return numOfTerms;
		default: 
			return 0;
	...
	private ArrayList<ArrayList<svm_node>> featureComponent(List<Paper> papers, FeatureType type, int offset){
		switch(type){
		case NumOfFiguresByPage: 
		case NumOfTablesByPage:
		case NumOfFormulasByPage: 
			return MetaDataFeatures.extractFeatureFromMetaDataMap(papers, type, offset);
		case TFIDF:
	...
	public svm_node[][] generateFeatureVectors(List<Paper> papers,List<FeatureType> types){
		int n = papers.size();
		...
		for(FeatureType type: types){
			ArrayList<ArrayList<svm_node>> fc= featureComponent(papers,type, offset);
			for(int i = 0; i < n; ++i){
